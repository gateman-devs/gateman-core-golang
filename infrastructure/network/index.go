...
func (network *NetworkController) Get(path string, headers *map[string]string, params *map[string]string) (response *[]byte, statusCode *int, err error) {
	...
	if err != nil {
		logger.Error("could not initiate GET request", logger.LoggerOptions{
			Key:  "error",
			Data: err,
		})
		return nil, nil, err
	...
	if err != nil {
		logger.Error("could not complete GET request", logger.LoggerOptions{
			Key:  "error",
			Data: err,
		})
		return nil, nil, err
	...
	if err != nil {
		logger.Error("could not read GET request body", logger.LoggerOptions{
			Key:  "error",
			Data: err,
		})
		return nil, nil, err
...
func (network *NetworkController) Post(path string, headers *map[string]string, body any, params *map[string]string, formdata bool, files *map[string][]byte) (response *[]byte, statusCode *int, err error) {
	...
	if formdata {
		...
		err = writer.Close()
		if err != nil {
			logger.Error("error closing writer after parsing formdata", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
			return nil, nil, err
		...
		if err != nil {
			logger.Error("error converting body to JSON", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
			return nil, nil, err
	...
	if err != nil {
		logger.Error("could not initiate POST request", logger.LoggerOptions{
			Key:  "error",
			Data: err,
		})
		return nil, nil, err
	...
	res, err := network.HttpClient.Do(req)
	if err != nil {
		logger.Error("could not complete POST request", logger.LoggerOptions{
			Key:  "error",
			Data: err,
		})
		return nil, nil, err
...
func (network *NetworkController) parseFormData(body any, writer *multipart.Writer, files *map[string][]byte) error {
	...
	for i, item := range any(body).(map[string]any) {
		...
		if err != nil {
			logger.Error("error parsing multipart payload", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
			return err
		...
		if err != nil {
			logger.Error("error marshalling item to byte for formdata", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
		}
		...
		if err != nil {
			logger.Error("error copying file to file part for formdata", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
			return err
	...
	for i, file := range *files {
		part, err := writer.CreateFormFile(i, utils.GenerateUULDString())
		if err != nil {
			logger.Error("error creating form file for formdata", logger.LoggerOptions{
				Key:  "error",
				Data: err,
			})
			return err
