...
func InitiateKeyExchange(ctx any, deviceID *string, clientPublicKey *ecdh.PublicKey, device_id *string, nonce *string) []byte {
	serverPrivateKey, err := ecdh.P256().GenerateKey(rand.Reader)
	if err != nil {
		apperrors.FatalServerError(ctx, err, device_id, nonce)
		return nil
	}
	...
	if !success {
		apperrors.FatalServerError(ctx, nil, device_id, nonce)
		return nil
	}
