...
func UserAuthenticationMiddleware(ctx *interfaces.ApplicationContext[any], intent *string, authToken string) (*interfaces.ApplicationContext[any], bool) {
	if authToken == "" {
		apperrors.AuthenticationError(ctx.Ctx, "missing auth token", *ctx.GetHeader("X-Device-Id"))
		return nil, false
	}
	validAccessToken, err := auth.DecodeAuthToken(authToken)
	if err != nil {
		apperrors.AuthenticationError(ctx.Ctx, "this session has expired", *ctx.GetHeader("X-Device-Id"))
		return nil, false
	}
	if !validAccessToken.Valid {
		apperrors.AuthenticationError(ctx.Ctx, "unauthorised access", *ctx.GetHeader("X-Device-Id"))
		return nil, false
	}
	...
	if !authTokenClaims["verifiedAccount"].(bool) {
		apperrors.AuthenticationError(ctx.Ctx, "verify your account before trying to use this route", *ctx.GetHeader("X-Device-Id"))
		return nil, false
	}
	...
	if ctx.DeviceID == "" {
		...
		apperrors.AuthenticationError(ctx.Ctx, "unauthorized access", *ctx.GetHeader("X-Device-Id"))
		return nil, false
	}
