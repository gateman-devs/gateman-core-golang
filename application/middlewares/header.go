...
func UserAgentMiddleware(ctx *interfaces.ApplicationContext[any], minAppVersion string, clientIP string) (*interfaces.ApplicationContext[any], bool) {
	...
	if agent == nil {
		apperrors.ClientError(ctx.Ctx, "why your user-agent header no dey? You be criminal?🤨", []error{errors.New("user agent header missing")}, nil, nil, nil)
		return nil, false
	}
	agentDetails := useragent.ParseUserAgent(*agent)
	fmt.Println(agentDetails)
	if agentDetails.Bot {
		apperrors.UnsupportedUserAgent(ctx.Ctx, ctx.DeviceID, ctx.Nonce)
		return nil, false
	}
	...
	if ctx.DeviceID == nil || *ctx.DeviceID == "" {
		apperrors.MalformedHeader(ctx.Ctx, nil, nil)
		return nil, false
	}
	ctx.AppVersion = ctx.GetHeader("X-App-Version")
	if ctx.AppVersion == nil || *ctx.AppVersion == "" {
		apperrors.MalformedHeader(ctx.Ctx, nil, nil)
		return nil, false
	}
	return ctx, true
