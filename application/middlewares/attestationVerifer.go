...
func AttestationVerifier(ctx *interfaces.ApplicationContext[any]) (*interfaces.ApplicationContext[any], bool) {
	...
	if attestationToken == nil {
		apperrors.AuthenticationError(ctx.Ctx, "attestation token missing", ctx.Nonce, ctx.DeviceID)
		return nil, false
	}
	...
	if err != nil {
		...
		apperrors.AuthenticationError(ctx.Ctx, "client verification failed", ctx.Nonce, ctx.DeviceID)
		return nil, false
	}
